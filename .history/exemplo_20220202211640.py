...
def second():
    ...
    dataArray = []
    print("Ficheiro Principal: ", len(fileData))
    for i in fileData:
    ...
    for i in dataToAdd2:
        content2.append(" "+dataToAdd2.read())
   # content_list2 = content2.split("\n")
    ...
    print("Ficheiro a adicionar: ",len(theRealData))

    list3 = [i + j for i, j in zip( theRealData,content2)]
    textfile = open("a_file.txt", "w")
    for index,value in enumerate(list3):
        textfile.write(value)
    textfile.close()

    print("Ficheiro Final: ",len(list3))
